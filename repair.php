...
/*-----------引入檔案區--------------*/
include "header.php";
if(empty($xoopsUser))redirect_header('index.php',3, _MD_TADREPAIR_NEED_LOGIN);
$xoopsOption['template_main'] = "tad_repair_repair_tpl.html";
...
function tad_repair_form($repair_sn=""){
  ...
  if($fixed_status==_MD_TADREPAIR_REPAIRED)redirect_header('index.php',3, _MD_TADREPAIR_CANT_MODIFY);
...
function insert_tad_repair(){
  ...
  $msg="";
	...
	if(empty($repair_name))$repair_name=XoopsUser::getUnameFromId($uid,0);

  $title=sprintf(_MD_TADREPAIR_MAIL_TITLE,$today,$_POST['repair_title']);
  $content=sprintf(_MD_TADREPAIR_MAIL_CONTENT,$repair_name,$today,$_POST['repair_title'],"<a href='".XOOPS_URL."/modules/tad_repair/index.php?repair_sn={$repair_sn}'>".XOOPS_URL."/modules/tad_repair/index.php?repair_sn={$repair_sn}</a>");
...
function update_tad_fixed($repair_sn=""){
  ...
  $msg="";
	...
	if(empty($fixed_name))$fixed_name=XoopsUser::getUnameFromId($uid,0);
...
switch($op){
  ...
  $main=tad_repair_form($repair_sn);
  break;

  //輸入表格
  case "tad_fixed_form":
  $main=tad_fixed_form($repair_sn);
  break;


  //預設動作
  default:
  $main=tad_repair_form();
  break;
}
