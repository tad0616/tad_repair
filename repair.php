...
if (!$_SESSION['can_repair']) {
    if (empty($xoopsModuleConfig['repair_group'])) {
        $error = _MD_TADREPAIR_NEED_SETUP;
    } else {
        $error = _MD_TADREPAIR_NOT_ALLOW_GROUP;
    }
    redirect_header(XOOPS_URL . '/modules/tad_repair/index.php', 10, $error);
}
